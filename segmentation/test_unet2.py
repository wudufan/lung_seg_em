...
parser.add_argument('--check_dir', type=str, default='../weights/unet2/b_9.0_k_0.5')
...
def prediction(file_tag, input_dir, sess, model, batch_size):
    ...
    @input_dir: input directory of the npz, output_dir will be input_dir/unet2
    '''
    
    filename = os.path.join(input_dir, file_tag+'.npz')
    pred_filename = os.path.join(input_dir, 'unet1', file_tag+'.npy')
    output_dir = os.path.join(input_dir, 'unet2')
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    
    # load training data
    imgs = np.load(filename)['img']
    masks = np.load(pred_filename)
